## ----------------------------------------------------------------------------
## 24. Q40-only response LCA sensitivity
##
## Purpose:
##   Test whether the major behavioral axes remain visible when the trigger /
##   risk-recognition items (Q41) are excluded from the active-responder LCA.
##
## Sample:
##   Active responders only (N = 501), from _shared/data_responder.rds
##
## Indicators:
##   Q40_1..Q40_8 binary response actions.
##
## Outputs:
##   Mplus_Q40_only_response/q40only_2..6class.*
##   _outputs/24_q40_only_fit.csv
##   _outputs/24_q40_only_item_response.csv
##   _outputs/24_q40_only_class_sizes.csv
##   _outputs/24_q40_only_summary.md
## ----------------------------------------------------------------------------
...
mp   <- file.path(adv, "Mplus_Q40_only_response")
...
ind_full <- paste0("Q40_", 1:8)
ind_short <- paste0("a", 1:8)
...
names(dat) <- ind_short
dat[is.na(dat)] <- -999
write.table(dat, file.path(mp, "q40_only_response_data.dat"),
            sep = " ", row.names = FALSE, col.names = FALSE, quote = FALSE)

write_inputs <- function(k) {
  names_str <- paste(ind_short, collapse = " ")
  inp <- paste0(
'TITLE: Q40-only response LCA k=', k, ';
DATA: FILE = "q40_only_response_data.dat";
VARIABLE:
  NAMES = ', names_str, ';
...
SAVEDATA:
  FILE = q40only_', k, 'class.dat;
  SAVE = CPROBABILITIES;
')
  writeLines(inp, file.path(mp, paste0("q40only_", k, "class.inp")))
}
invisible(lapply(2:6, write_inputs))

force_mplus <- as.logical(Sys.getenv("FORCE_MPLUS", "FALSE"))
run_mplus_one <- function(k) {
  inp <- paste0("q40only_", k, "class.inp")
  out_file <- file.path(mp, paste0("q40only_", k, "class.out"))
  cprob_file <- file.path(mp, paste0("q40only_", k, "class.dat"))
...
parse_enum <- function(k) {
  f <- file.path(mp, paste0("q40only_", k, "class.out"))
...
write.csv(fit, file.path(out, "24_q40_only_fit.csv"), row.names = FALSE)
...
write.csv(icp, file.path(out, "24_q40_only_item_response.csv"), row.names = FALSE)

class_sizes <- bind_rows(lapply(2:6, function(k) {
  f <- file.path(mp, paste0("q40only_", k, "class.dat"))
...
write.csv(class_sizes, file.path(out, "24_q40_only_class_sizes.csv"), row.names = FALSE)

sink(file.path(out, "24_q40_only_summary.md"))
cat("# Q40-Only Response LCA Sensitivity\n\n")
cat("Sample: active responders only, N =", nrow(r), "\n\n")
cat("Indicators: Q40_1..Q40_8 response actions only.\n\n")
cat("Purpose: this sensitivity was not intended to reproduce the exact number of primary classes, ")
cat("but to examine whether major behavioral axes remain visible when trigger/risk-recognition items are excluded.\n\n")
cat("## Fit table\n\n")
print(fit)
...
cat("Wrote:\n",
    file.path(out, "24_q40_only_fit.csv"), "\n",
    file.path(out, "24_q40_only_item_response.csv"), "\n",
    file.path(out, "24_q40_only_class_sizes.csv"), "\n",
    file.path(out, "24_q40_only_summary.md"), "\n")
